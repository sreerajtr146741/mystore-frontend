        ...
        <div class="card border-0 shadow-sm rounded-4">
          <div class="card-body p-0">
            ...
            <?php foreach($cart as $pid => $item): ?>
              <?php
                if (!is_array($item)) continue; // skip bad data
                $qty = (int)($item['qty'] ?? 1);
              ...
              <div class="p-3 p-md-4 border-bottom">
                <div class="d-flex align-items-center gap-3">
                  <img class="item-img" src="<?= !empty($image) ? $image : 'https://img.icons8.com/ios-filled/100/0d6efd/box.png' ?>" alt="">
                  <div class="flex-grow-1">
                    ...
                    <div class="mt-1 d-flex align-items-center gap-3 flex-wrap">
                      ...
                      <!-- Per-item Buy Now (Optional, removing if route doesn't exist, but kept for now) -->
                      <!-- Assuming checkout.single might not exist in index.php route map, better to link to generic checkout or remove -->
                      <!-- <a href="<?= route('checkout.index') ?>" class="btn btn-sm btn-buy px-3 py-1 rounded-pill"><i class="bi bi-lightning-charge-fill me-1"></i> Buy Now</a> -->
                      
                      <!-- Remove Button -->
                      <a href="<?= route('cart.index', ['action'=>'remove', 'id'=>$pid]) ?>" class="text-danger small text-decoration-none">Remove</a>
                    </div>
                  </div>
                  <div class="text-end">
